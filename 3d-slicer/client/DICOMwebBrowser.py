...
Disable if data is added or removed from the database."""
  ...
  def connectToServer(self):
    ...
    hostname = qt.QUrl(self.serverUrl).host()
    ...
    if os.path.exists(config_path):
      ...
      if config.has_section(hostname):
        userId = config.get(hostname, 'user', fallback=None)
        userPassword = config.get(hostname, 'password', fallback=None)
        logging.info(f"[Auth] config.ini에서 [{hostname}] 인증정보 로드됨 (user={userId})")
      else:
        logging.debug(f"[Auth] config.ini에 [{hostname}] 섹션 없음, 인증 없이 접속")

    if userId and userPassword:
      from requests.auth import HTTPBasicAuth
      from dicomweb_client.session_utils import create_session_from_auth
      auth = HTTPBasicAuth(userId, userPassword)
      session = create_session_from_auth(auth)

    elif "googleapis.com" in self.serverUrl:
